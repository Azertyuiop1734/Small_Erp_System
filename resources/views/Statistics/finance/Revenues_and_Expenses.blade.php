...
<style>
    ...
    * { font-family: 'Cairo', 'Tajawal', sans-serif; }
    ...
    .fin-page::before {
        ...
        background:
            radial-gradient(ellipse 80% 60% at 15% 10%,  rgba(99,102,241,.09) 0%, transparent 60%),
            radial-gradient(ellipse 60% 50% at 85% 85%,  rgba(16,185,129,.07) 0%, transparent 60%),
            radial-gradient(ellipse 40% 40% at 55% 30%,  rgba(244,63,94,.06)  0%, transparent 50%);
    }
    .dark .fin-page::before {
        background:
            radial-gradient(ellipse 80% 60% at 15% 10%,  rgba(99,102,241,.14) 0%, transparent 60%),
            radial-gradient(ellipse 60% 50% at 85% 85%,  rgba(16,185,129,.11) 0%, transparent 60%),
            radial-gradient(ellipse 40% 40% at 55% 30%,  rgba(244,63,94,.09)  0%, transparent 50%);
    }
    ...
    .orb-1 { width:340px;height:340px; background:rgba(99,102,241,.07);  top:-90px;  right:-90px; animation-delay:0s;  }
    .orb-2 { width:260px;height:260px; background:rgba(16,185,129,.06);  bottom:10%; left:-60px;  animation-delay:-5s; }
    .orb-3 { width:200px;height:200px; background:rgba(244,63,94,.05);   top:40%;    right:3%;    animation-delay:-9s; }
    .dark .orb-1 { background:rgba(99,102,241,.13); }
    .dark .orb-2 { background:rgba(16,185,129,.11); }
    .dark .orb-3 { background:rgba(244,63,94,.09);  }
    ...
    .top-card::before { content:'';position:absolute;top:-70px;right:-70px;width:240px;height:240px;background:rgba(99,102,241,.05);border-radius:50%;filter:blur(70px);pointer-events:none;display:none; }
    ...
    .top-icon {
        ...
        box-shadow:0 8px 28px rgba(99,102,241,.38),0 0 0 3px rgba(99,102,241,.15);
        position:relative;
    }
    ...
    .top-sub span { color:#6366F1;font-weight:900; }
    .dark .top-sub span { color:#818cf8; }
    ...
    .btn-print:hover { color:#6366F1; border-color:rgba(99,102,241,.35); box-shadow:0 4px 20px rgba(99,102,241,.18); transform:translateY(-2px); }
    ...
    .stat-value span { font-size:.7rem;font-weight:700;color:#94a3b8;margin-right:.3rem; }
    ...
    .stat-card.rose  .stat-label    { color:#F43F5E; }
    ...
    .stat-card.rose  .stat-icon-box { background:rgba(244,63,94,.10);  color:#F43F5E; }
    ...
    .dark .stat-card.rose  .stat-icon-box { background:rgba(244,63,94,.18);  }
    ...
    .chart-card::before { content:'';position:absolute;top:-80px;right:-80px;width:280px;height:280px;background:rgba(99,102,241,.05);border-radius:50%;filter:blur(80px);pointer-events:none;display:none; }
    .dark .chart-card::before { display:block; }

    .chart-strip {
        background:linear-gradient(135deg,rgba(99,102,241,.07) 0%,rgba(79,70,229,.05) 100%);
        border-bottom:1px solid rgba(99,102,241,.09);
        padding:1rem 2rem; display:flex; align-items:center; gap:.7rem;
    }
    .dark .chart-strip { background:linear-gradient(135deg,rgba(99,102,241,.11) 0%,rgba(79,70,229,.08) 100%); border-bottom-color:rgba(51,65,85,.4); }
    .strip-dot { width:8px;height:8px;border-radius:50%;background:#6366F1;box-shadow:0 0 9px rgba(99,102,241,.7);flex-shrink:0; }
    .strip-label { font-size:.71rem;font-weight:900;color:#6366F1;text-transform:uppercase;letter-spacing:.1em; }
    .dark .strip-label { color:#818cf8; }
    ...
    .legend-item { display:flex;align-items:center;gap:.5rem;font-size:.78rem;font-weight:700;color:#64748b; }
    ...
    .ca-bar { height:4px; }
    .ca-bar.success { background:linear-gradient(90deg,#6366F1,#4F46E5); }
    ...
    .ca-ring.success { background:radial-gradient(circle at 35% 35%,rgba(99,102,241,.22),rgba(99,102,241,.06));border:1.5px solid rgba(99,102,241,.3);box-shadow:0 0 0 8px rgba(99,102,241,.07); }
    ...
    .ca-cc { stroke:#6366F1;stroke-width:2.5;fill:none;stroke-dasharray:166;stroke-dashoffset:166; }
    .ca-cm { stroke:#6366F1;stroke-width:3;fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:48;stroke-dashoffset:48; }
    ...
    .ca-prog-fill { height:100%;border-radius:99px;background:linear-gradient(90deg,#6366F1,#4F46E5);transform-origin:left; }
    ...
    .ca-btn.success { background:linear-gradient(135deg,#6366F1,#4F46E5);color:#fff;box-shadow:0 5px 18px rgba(99,102,241,.38); }
    .ca-btn.success:hover { transform:translateY(-2px) scale(1.04);box-shadow:0 8px 26px rgba(99,102,241,.55); }
...
<div class="page-container">
    ...
    <div class="glass top-card">
        <div class="top-brand">
            <div class="top-icon"><i class="fas fa-chart-pie"></i></div>
            <div>
                <h1 class="top-title">النظرة المالية العامة</h1>
    ...
    <div class="stats-grid">
        ...
        <div class="glass stat-card blue">
            <div class="stat-top">
                <div>
                    <p class="stat-label">مبيعات اليوم</p>
                    <p class="stat-value"><span>DA</span>{{ number_format($salesToday, 2) }}</p>
                </div>
                <div class="stat-icon-box"><i class="fas fa-calendar-day"></i></div>
            </div>
            <div class="stat-bg-icon" style="color:#3B82F6"><i class="fas fa-calendar-day"></i></div>
        </div>
        ...
        <div class="glass stat-card green">
            <div class="stat-top">
                <div>
                    ...
                    <p class="stat-value"><span>DA</span>{{ number_format($salesThisMonth, 2) }}</p>
                </div>
                <div class="stat-icon-box"><i class="fas fa-chart-line"></i></div>
            </div>
            <div class="stat-bg-icon" style="color:#10B981"><i class="fas fa-chart-line"></i></div>
        </div>

        <!-- Monthly expenses -->
        <div class="glass stat-card rose">
            <div class="stat-top">
                <div>
                    <p class="stat-label">مصاريف الشهر</p>
                    <p class="stat-value"><span>DA</span>{{ number_format($expensesThisMonth, 2) }}</p>
                </div>
                <div class="stat-icon-box"><i class="fas fa-wallet"></i></div>
            </div>
            <div class="stat-bg-icon" style="color:#F43F5E"><i class="fas fa-wallet"></i></div>
        </div>
    ...
    <div class="glass chart-card">
        <div class="chart-strip">
            ...
            <span class="strip-label">الأداء المالي السنوي</span>
        ...
        <div class="chart-legend">
            <div class="legend-item">
                <div class="legend-dot" style="background:#3B82F6;box-shadow:0 0 6px rgba(59,130,246,.5)"></div>
                إجمالي المبيعات
            </div>
            <div class="legend-item">
                <div class="legend-dot" style="background:#F43F5E;box-shadow:0 0 6px rgba(244,63,94,.5)"></div>
                إجمالي المصاريف
            </div>
        </div>
...
const salesGrad = ctx.createLinearGradient(0, 0, 0, 420);
salesGrad.addColorStop(0, 'rgba(59,130,246,.45)');
salesGrad.addColorStop(1, 'rgba(59,130,246,0)');

const expGrad = ctx.createLinearGradient(0, 0, 0, 420);
expGrad.addColorStop(0, 'rgba(244,63,94,.40)');
        ...
        datasets: [
            {
                ...
                pointBorderColor: '#3B82F6', pointBorderWidth: 2.5
            },
            {
                label: 'المصاريف (DA)',
                ...
                pointRadius: 5, pointHoverRadius: 7,
                pointBackgroundColor: '#fff',
                pointBorderColor: '#F43F5E', pointBorderWidth: 2.5
            }
        ]
    },
    options: {
        ...
        plugins: {
            ...
            tooltip: {
                backgroundColor: isDark() ? 'rgba(10,17,34,.95)' : 'rgba(255,255,255,.95)',
                titleColor: isDark() ? '#f1f5f9' : '#1e293b',
                bodyColor:  isDark() ? '#94a3b8'  : '#64748b',
                borderColor: isDark() ? 'rgba(51,65,85,.5)' : 'rgba(255,255,255,.8)',
                borderWidth: 1,
                ...
                callbacks: {
                    label: ctx => ' ' + ctx.dataset.label + ': ' + ctx.parsed.y.toLocaleString() + ' DA'
                }
            }
        },
        scales: {
            y: {
                ...
                ticks: {
                    ...
                    callback: v => v.toLocaleString() + ' DA'
                }
            },
            x: {
                grid: { display: false },
